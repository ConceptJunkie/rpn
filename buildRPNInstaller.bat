...
iff %_X64 eq 1 then
    ...
    move Output\setup_rpnChilada.exe "%RPN_TARGET%\setup_rpnChilada-%VERSION%-win64.exe"
else
    copy msvcp100-win32.dll build\exe.win32-3.6\msvcp100.dll
    "%PROG32_DIR%\Inno Setup 5\ISCC.exe" rpn32.iss
    move Output\setup_rpnChilada.exe "%RPN_TARGET%\setup_rpnChilada-%VERSION%-win32.exe"
endiff
